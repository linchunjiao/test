	...
	<view class="mask" @tap="tapMask" :class="!value?'':'mask-show'" @touchmove.stop.prevent>
		...
		<block v-if="type=='input'">
			<view class="input-view" @tap.stop>
				<view v-if="mData.title" class="title">{{mData.title}}</view>
				<view class="content">
					<view v-for="(item,index) in mData.content" :key="index" class="input-box">
						...
						<block v-if="item.type=='textarea'">
							...
							:style="{minHeight: item.minHeight ? item.minHeight : ''}" 
							:disabled="item.disabled" 
							:placeholder="item.placeholder"
							auto-height />
						</block>
						<block v-if="item.type=='radio'">
							<radio-group @change="radioChange($event,item.radios)">
							    <label style="margin-right: 30upx;" class="uni-list-cell uni-list-cell-pd" v-for="(radio, index) in item.radios" :key="radio.value">
